...
const VAR_NO_EXPRESSION: MockVariable[] = [{ id: 'v1', name: 'taxAmount', expression: '' }];
const VAR_PARTIAL_EXPRESSION: MockVariable[] = [{ id: 'v1', name: 'taxAmount', expression: 'Price' }];
const VAR_WITH_EXPRESSION: MockVariable[] = [{ id: 'v1', name: 'taxAmount', expression: 'Price * 0.1' }];
...
export const VariableMockup: FunctionComponent<VariableMockupProps> = ({ step }) => {
  ...
  const allMappingLines: MappingLine[] = useMemo(
    () => [
      { sourceId: 'src-price', targetId: 'tgt-taxAmount' },
      { sourceId: 'src-price', targetId: 'tgt-subtotal' },
      { sourceId: 'src-taxAmount', targetId: 'tgt-subtotal' },
    ],
    [],
  );
  ...
  }, [variables, isTargetExpanded, isSubtotalExpanded, mappingLines, hideMappingLines, persistentLines]);

  const containerRect = containerRef.current?.getBoundingClientRect();

  const handleAddVar = useCallback((name: string) => {
    setVariables((prev) => [...prev, { id: `v-${Date.now()}`, name, expression: '' }]);
          ...
          <ExpansionPanels firstPanelId="variables-header" lastPanelId="source-body">
            ...
            {variables.map((variable) => (
              <ExpansionPanel
                ...
                id={`var-${variable.id}`}
                summary={
                  <Tooltip content={variable.expression || '(empty expression)'} position="right">
                    <div className="variable-row" id={`src-${variable.name}`}>
                      <Icon>
                        <Draggable />
                      </Icon>
                          ...
                          {variables.map((variable) =>
                            renamingVarInTargetId === variable.id ? (
                              ...
                              <div key={variable.id} className="node__container">
                                <div id={`tgt-${variable.name}`} className="node__header">
