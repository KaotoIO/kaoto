...
export class BeansXmlParser {
  static async transformBeanFactory(beanElement: Element): Promise<BeanFactory> {
    ...
    const properties = (await DynamicCatalogRegistry.get().getEntity(CatalogKind.Processor, 'beanFactory'))?.properties;
