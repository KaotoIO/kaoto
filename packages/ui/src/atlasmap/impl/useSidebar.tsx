...
*/
import { IAtlasmapField, IMappingDetailsViewProps, MappingDetailsView } from '../Views';
...
export function useSidebar({
  ...
  return useCallback(() => {
    if (selectedMapping) {
      ...
      const sources = m.getMappedFields(true).map(fromMappedFieldToIMappingField);
      const targets = m.getMappedFields(false).map(fromMappedFieldToIMappingField);
      const showSourcesIndex = sources.length > 1 && m.transition.isManyToOneMode() && !m.transition.enableExpression;
      const showTargetsIndex = targets.length > 1 && m.transition.isOneToManyMode() && !m.transition.enableExpression;
      ...
      if (multiplicityFieldAction) {
        ...
        multiplicity = {
          ...
          transformationsArguments: multiplicityFieldAction.argumentValues.map((a) => ({
            label: CommonUtil.toDisplayable(a.name),
            name: a.name,
            type: multiplicityFieldAction.definition?.arguments.find((arg) => arg.name === a.name)?.type,
            value: a.value,
            options: a.name === 'delimiter' ? delimitersOptions : undefined,
          })),
          onChange: (name) => handleMultiplicityChange(multiplicityFieldAction, name),
          onArgumentChange: (argumentName, arguemntValue) =>
            handleMultiplicityArgumentChange(multiplicityFieldAction, argumentName, arguemntValue),
        };
      }

      const handleRemoveMappedField = (isSource: boolean, index: number) => {
        const amField = selectedMapping.mapping.getMappedFieldForIndex('' + (index + 1), isSource);
        if (amField) {
       ...
       */
      const addableSources = [constants?.fields, sourceProperties?.fields, flatSources]
      ...
      return (
        <MappingDetailsView
          notifications={notifications.filter((n) => n.mappingId === selectedMapping.id && !n.isRead)}
          sources={sources}
          targets={targets}
