...
export type Views = 'ColumnMapper' | 'MappingTable' | 'NamespaceTable' | 'FreeView';
...
export function useContextToolbar({
  ...
  const { state: showMappingPreview, toggle: toggleShowMappingPreview } = useToggle(false, amToggleMappingPreview);
  const { state: showTypes, toggle: toggleShowTypes } = useToggle(true);
  const { state: showMappedFields, toggle: toggleShowMappedFields } = useToggle(true, amToggleShowMappedFields);
  const { state: showUnmappedFields, toggle: toggleShowUnmappedFields } = useToggle(true, amToggleShowUnmappedFields);
  ...
  const contextToolbar = useMemo(
    () => (
      <ContextToolbar>
        ...
        {(showColumnMapperViewToolbarItem || showMappingTableViewToolbarItem || showNamespaceTableViewToolbarItem) && (
        ...
        <ToolbarGroup variant="icon-button-group">
          {showToggleMappingPreviewToolbarItem && (activeView === 'ColumnMapper' || activeView === 'MappingTable') && (
            <ToggleMappingPreviewToolbarItem toggled={showMappingPreview} onClick={toggleShowMappingPreview} />
          )}
          {showToggleTypesToolbarItem && (activeView === 'ColumnMapper' || activeView === 'MappingTable') && (
            <ToggleTypesToolbarItem toggled={showTypes} onClick={toggleShowTypes} />
          )}
          {showToggleMappedFieldsToolbarItem && activeView === 'ColumnMapper' && (
            <ToggleMappedFieldsToolbarItem toggled={showMappedFields} onClick={toggleShowMappedFields} />
          )}
          {showToggleUnmappedFieldsToolbarItem && activeView === 'ColumnMapper' && (
            <ToggleUnmappedFieldsToolbarItem toggled={showUnmappedFields} onClick={toggleShowUnmappedFields} />
          )}
          {showAddNewMappingToolbarItem && (activeView === 'ColumnMapper' || activeView === 'MappingTable') && (
