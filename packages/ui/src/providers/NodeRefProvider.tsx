...
import { useBoundingCanvasRect } from '../_bk_atlasmap/UI/Canvas/useBoundingCanvasRect';
...
export const NodeRefProvider: FunctionComponent<PropsWithChildren> = ({ children }) => {
  ...
  const unsetRef = (id: string | undefined | Array<string | undefined>, key: string) => {
    const ids = Array.isArray(id) ? id : [id];
    for (let index = 0; index < ids.length; index++) {
      const curId = ids[index];
...
export function useNodeRect() {
  ...
  const getRect = useCallback(
    (id: string): NodeRect | null => {
      ...
      if (element) {
        const dimensions = getBoundingCanvasRect(element);
        if (dimensions.height === 0 && node.parentId) {
          return getRect(node.parentId);
        }
        let boundaryRect;
        if (node.boundaryId && (boundaryRect = getRect(node.boundaryId))) {
          const overrideWidthRect = node.overrideWidth ? getRect(node.overrideWidth) : undefined;
          const overrideHeightRect = node.overrideHeight ? getRect(node.overrideHeight) : undefined;
          const width = overrideWidthRect?.width || dimensions.width;
          const height = overrideHeightRect?.height || dimensions.height;
          const x = Math.min(
            Math.max(overrideWidthRect?.left || dimensions.left, boundaryRect.left),
            boundaryRect.width + boundaryRect.left - dimensions.width,
          );
          const isXClipped = x !== (overrideWidthRect?.x || dimensions.x);
          const y = Math.min(
            Math.max(overrideHeightRect?.top || dimensions.top + dimensions.height / 2, boundaryRect.top),
            boundaryRect.height + boundaryRect.top,
          );
          const isYClipped = y === boundaryRect.top || y === boundaryRect.height + boundaryRect.top;
          return {
            width,
            height,
            x,
