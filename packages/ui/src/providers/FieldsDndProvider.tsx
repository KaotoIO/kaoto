...
*/
import { FunctionComponent, createContext, PropsWithChildren, useRef } from 'react';
...
const probablyTouch =
  window.matchMedia &&
  (window.matchMedia('(pointer: none)').matches || window.matchMedia('(pointer: coarse)').matches) &&
...
export const FieldsDndContext = createContext<IFieldsDndContext | null>(null);
...
export const FieldsDndProvider: FunctionComponent<PropsWithChildren> = ({ children }) => {
  ...
  const setHoveredTarget = (target: IDragAndDropField | null) => (hoveredTarget.current = target);
  const Provider = probablyTouch ? TouchAndPointerProvider : MouseOnlyProvider;
  return (
    <Provider>
      <FieldsDndContext.Provider value={{ getHoveredTarget, setHoveredTarget }}>{children}</FieldsDndContext.Provider>
    </Provider>
  );
};
