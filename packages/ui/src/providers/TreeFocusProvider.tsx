...
export const TreeFocusProvider: FunctionComponent<PropsWithChildren> = ({ children }) => {
  const [focusedItem, setFocusedItem] = useState<HTMLElement | null>(null);
  return <TreeFocusContext.Provider value={{ focusedItem, setFocusedItem }}>{children}</TreeFocusContext.Provider>;
};
...
export const useTreeFocus = ({
  ...
  const getFocusableItems = (el: HTMLElement) => {
    return Array.from<HTMLElement>(el.closest('[role=tree]')?.querySelectorAll('[role=treeitem]') || []).filter(
      (el) => {
  ...
  return {
    ...
    handlers: {
      ...
      onBlur: (event: FocusEvent<HTMLElement>) => {
        if (
          (event.relatedTarget as HTMLElement | null)?.closest('[role=tree]') !== ref.current?.closest('[role=tree]')
        ) {
          setFocusedItem(null);
        }
      },
