 ...
 * - kaoto.testingCatalog.version: { version: "4.10.1", runtime: "Citrus" }
 */
...
export interface AbstractSettingsAdapter {
  ...
  saveSettings(settings: ISettingsModel): void | Promise<void>;
}

export class SettingsModel implements ISettingsModel {
  catalogUrl: string = '';
  camelCatalog: CatalogVersion = { version: '', runtime: CAMEL_RUNTIMES[0] };
  testingCatalog: CatalogVersion = { version: '', runtime: TEST_RUNTIMES[0] };
  ...
  constructor(options: Partial<ISettingsModel> = {}) {
    ...
    const { rest, camelCatalog, testingCatalog, ...topLevel } = options;
    ...
    if (testingCatalog) {
      this.testingCatalog = testingCatalog;
    }
  }
}
