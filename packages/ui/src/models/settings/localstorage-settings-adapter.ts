import { isTestRuntime, resolveSettingsForCatalogUrl } from '../../utils/catalog-helper';
...
export class LocalStorageSettingsAdapter implements AbstractSettingsAdapter {
    ...
    try {
      ...
      if (oldCatalog && oldCatalog !== 'undefined') {
        ...
        if (parsed?.version && parsed.runtime) {
          ...
          if (isTestRuntime(parsed.runtime) && !parsedSettings.testingCatalog) {
            parsedSettings.testingCatalog = catalogVersion;
  ...
  async saveSettings(settings: ISettingsModel): Promise<void> {
    const resolvedSettings = await resolveSettingsForCatalogUrl(settings);
    localStorage.setItem(LocalStorageKeys.Settings, JSON.stringify(resolvedSettings));
  }
}
