...
describe('NodeSchemaResolver.getProcessorSchema', () => {
  ...
  it('should resolve schema for entity processors', async () => {
    const mockSchema: KaotoSchemaDefinition['schema'] = {
      type: 'object',
      properties: {
        uri: { type: 'string' },
    ...
    const result = await NodeSchemaResolver.getProcessorSchema('from', {});

    expect(DynamicCatalogRegistry.get().getEntity).toHaveBeenCalledWith(CatalogKind.Entity, 'from');
    expect(result).toEqual(mockSchema);
  });
