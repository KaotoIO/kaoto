...
import { BODY_DOCUMENT_ID, DocumentDefinition, DocumentDefinitionType } from '../models/datamapper';
...
export class JsonSchemaDocumentService {
  ...
  static createJsonSchemaDocument(
    definitionOrDocumentType: DocumentDefinition | DocumentType,
    documentId?: string,
    content?: string,
  ) {
    let definition: DocumentDefinition;

    if (definitionOrDocumentType instanceof DocumentDefinition) {
      definition = definitionOrDocumentType;
    } else {
      definition = new DocumentDefinition(
        definitionOrDocumentType,
        DocumentDefinitionType.JSON_SCHEMA,
        documentId === BODY_DOCUMENT_ID ? undefined : documentId,
        { [`${documentId || 'schema'}.json`]: content! },
      );
    }
