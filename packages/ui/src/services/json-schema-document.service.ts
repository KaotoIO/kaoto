...
export class JsonSchemaField extends BaseField {
  ...
  adopt(parent: IField) {
    ...
    this.namedTypeFragmentRefs.forEach(
      (ref) => !existing.namedTypeFragmentRefs.includes(ref) && existing.namedTypeFragmentRefs.push(ref),
    );
    this.fields.forEach((child) => child.adopt(existing));
    return existing;
  }
