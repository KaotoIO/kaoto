  ...
  describe('without pre-populated mappings', () => {
    ...
    describe('engageMapping()', () => {
      ...
      it('should generate correct XPath for fields with xs:extension', () => {
        const adtInDoc = TestUtil.createAdtInDoc();
        const adtOutDoc = TestUtil.createAdtOutDoc();
        const adtTree = new MappingTree(
          adtOutDoc.documentType,
          adtOutDoc.documentId,
          DocumentDefinitionType.XML_SCHEMA,
        );
        const adtInDocNode = new DocumentNodeData(adtInDoc);
        const adtOutDocNode = new TargetDocumentNodeData(adtOutDoc, adtTree);
      ...
      it('should generate correct XPath for attributes with xs:extension', () => {
        ...
        const sourceDoc = XmlSchemaDocumentService.createXmlSchemaDocument(extensionSimpleDef);
        const targetDoc = XmlSchemaDocumentService.createXmlSchemaDocument(extensionSimpleDef);
