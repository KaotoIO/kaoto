...
describe('JsonSchemaDocumentUtilService', () => {
  describe('getChildField()', () => {
    it('should find child field by key', () => {
      ...
      parent.fields.push(
        { key: 'child1', type: Types.String, namespaceURI: 'test-namespace' } as JsonSchemaField,
        { key: 'child2', type: Types.Integer, namespaceURI: 'test-namespace' } as JsonSchemaField,
      );

      const result = JsonSchemaDocumentUtilService.getChildField(parent, Types.String, 'child1', 'test-namespace');
    ...
    it('should return undefined when key does not match', () => {
      const doc = new JsonSchemaDocument(DocumentType.SOURCE_BODY, 'test-doc');
      const parent = new JsonSchemaField(doc, 'parent', Types.Container);
      parent.fields.push({ key: 'child1', type: Types.String, namespaceURI: 'test-namespace' } as JsonSchemaField);

      const result = JsonSchemaDocumentUtilService.getChildField(parent, Types.String, 'nonexistent', 'test-namespace');
    ...
    it('should return undefined when type does not match', () => {
      ...
      const result = JsonSchemaDocumentUtilService.getChildField(parent, Types.Integer, 'child1', 'test-namespace');
