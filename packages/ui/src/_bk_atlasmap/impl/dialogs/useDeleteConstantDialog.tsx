...
export function useDeleteConstantDialog(): [ReactElement, (constName: string) => void] {
  const { deleteConstant } = useDataMapper();
  const [deleteConstantDialog, openDeleteConstantDialog] = useConfirmationDialog(
    'Delete constant?',
  ...
  const onDeleteConstant = useCallback(
    (constName: string) => openDeleteConstantDialog(() => deleteConstant(constName)),
