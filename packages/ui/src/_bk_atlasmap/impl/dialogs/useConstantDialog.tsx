...
import { constantTypes } from '../../core';
...
export function useConstantDialog(
  title: string,
): [ReactElement, (cb: ConstantCallback, constants: IDocument | null, constant?: IConstant) => void] {
  const [onConstantCb, setOnConstantCb] = useState<ConstantCallback | null>(null);
  const [initialConstant, setInitialConstant] = useState<IConstant | null>({
    name: '',
    value: '',
    valueType: constantTypes[0][0],
  });
  const [constantsDoc, setConstants] = useState<IDocument | null>(null);
  ...
  function onValidation(name: string): boolean {
    ...
    if (constantsDoc) {
      ...
      const fields = constantsDoc.fields.filter((fieldOrGroup) => fieldOrGroup.name === name);
  ...
  const onOpenConstantDialog = useCallback(
    (callback: ConstantCallback, constantsDoc: IDocument | null, constant?: IConstant) => {
