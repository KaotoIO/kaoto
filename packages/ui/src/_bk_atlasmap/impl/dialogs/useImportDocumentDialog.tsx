...
import { useConfirmationDialog } from '../../../components/dialogs/useConfirmationDialog';
...
export function useImportDocumentDialog(): [ReactElement, (selectedFile: File, isSource: boolean) => void] {
  ...
  const [parametersDialog, openParametersDialog] = useParametersDialog('Select CSV Processing Parameters');
  const [defaultSchema, setDefaultSchema] = useState(false);
  const [specifyInstanceSchemaDialog, openSpecifyInstanceSchema] = useSpecifyInstanceSchemaDialog(defaultSchema);

  const importFile = useCallback(
    (selectedFile: File, isSource: boolean) => {
      if (selectedFile.name) {
        const userFileSplit = selectedFile.name.split('.');
        const userFileSuffix: string = userFileSplit[userFileSplit.length - 1].toUpperCase();
