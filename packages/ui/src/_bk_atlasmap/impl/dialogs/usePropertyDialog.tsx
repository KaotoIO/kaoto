...
import { propertyTypes } from '../../core';
...
export function usePropertyDialog(
...
): [ReactElement, (cb: PropertyCallback, properties: IDocument | null, property?: IProperty) => void] {
  const [onPropertyCb, setOnPropertyCb] = useState<PropertyCallback | null>(null);
  const [initialProperty, setInitialProperty] = useState<IProperty | null>({
    name: '',
    valueType: propertyTypes[0][0],
    scope: scopeOptions[0].value,
  });
  const [properties, setProperties] = useState<IDocument | null>(null);
  ...
  function onValidation(name: string, scope: string): boolean {
    ...
    if (properties) {
      ...
      const fields = properties.fields.filter(
        (fieldOrGroup) => fieldOrGroup.name === name && fieldOrGroup.amField.scope === scope,
      );
      fieldNameExists = fields.length > 0;
    }
    return (name === initialProperty?.name && scope === initialProperty.scope) || !fieldNameExists;
  }
  const dialog = (
    <PropertyDialog
      title={title}
  ...
  const onOpenPropertyDialog = useCallback(
    (callback: PropertyCallback, properties: IDocument | null, property?: IProperty) => {
