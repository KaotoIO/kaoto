...
import { IParameter, getCsvParameterOptions } from '../core';
...
export function useDataMapperDialogs({ modalContainer }: IUseAtlasmapDialogsProps) {
  const {
    configModel,
    selectedMapping,
    createConstant,
  ...
  const [createConstantDialog, openCreateConstantDialog] = useConstantDialog('Create Constant');
  const onCreateConstant = useCallback(
    (constants: IDocument | null, addToActiveMapping?: boolean) => {
  ...
  const [editConstantDialog, openEditConstantDialog] = useConstantDialog('Edit Constant');
  const onEditConstant = useCallback(
    (constant: IConstant, constants: IDocument | null) => {
  ...
  const [scopeOptions, setScopeOptions] = useState(getPropertyScopeOptions(true));
  const [createPropertyDialog, openCreatePropertyDialog] = usePropertyDialog('Create Property', scopeOptions);
  const onCreateProperty = useCallback(
    (isSource: boolean, properties: IDocument | null, addToActiveMapping?: boolean) => {
  ...
  const [editPropertyDialog, openEditPropertyDialog] = usePropertyDialog('Edit Property', scopeOptions);
  const onEditProperty = useCallback(
    (property: IProperty, isSource: boolean, properties: IDocument | null) => {
      setScopeOptions(getPropertyScopeOptions(isSource));
  ...
  const [captureDocumentNameToast, onCaptureDocumentID] = useCaptureDocumentIDToast();

  const [changeDocumentNameDialog, openChangeDocumentNameDialog] = useChangeDocumentNameDialog();
  ...
  const [createNamespaceDialog, openCreateNamespaceDialog] = useNamespaceDialog('Create namespace');
  const onCreateNamespace = useCallback(
    (docName: string) => {
      openCreateNamespaceDialog(({ alias, uri, locationUri, targetNamespace }) =>
        createNamespace(docName, alias, uri, locationUri, targetNamespace),
      );
    },
    [createNamespace, openCreateNamespaceDialog],
  );
  const [editNamespaceDialog, openEditNamespaceDialog] = useNamespaceDialog('Edit namespace');
    ...
    (docName: string, namespace: INamespace) => {
      openEditNamespaceDialog(
        ({ alias, uri, locationUri, targetNamespace }) =>
          editNamespace(docName, namespace.alias, alias, uri, locationUri, targetNamespace),
        namespace,
      );
    },
  ...
  const [removeMappedFieldDialog, onRemoveMappedField] = useRemoveMappedFieldDialog();
  ...
  const [createEnableCustomClassDialog, openCreateEnableCustomClassDialog] = useCustomClassDialog(
    'Load Java Document From Custom Class',
  );
  const onEnableCustomClass = useCallback(
    (isSource: boolean): void => {
      openCreateEnableCustomClassDialog(({ customClassName, collectionType }) =>
        enableCustomClass(configModel, customClassName, collectionType, isSource),
      );
    },
  ...
  const [editCSVParamsDialog, openEditCSVParamsDialog] = useParametersDialog('Edit CSV Processing Parameters');
  function initialCSVParams(docId: string, isSource: boolean): IParameter[] {
    // User-defined CSV parameters.
    const predefinedParameters: { [key: string]: string } = getDocCSVParams(docId, isSource);

    // Complete list of available CSV parameters.
    const initialCSVParameters = getCsvParameterOptions();
    const predefinedParamNames = Object.keys(predefinedParameters).map((key) => key);
    const predefinedParamValues = Object.values(predefinedParameters).map((key) => key);
    ...
    for (const { index } of initialCSVParameters.map((value, index) => ({
      ...
      for (const { pvalue, pindex } of predefinedParamNames.map((pvalue, pindex) => ({
        pindex,
        pvalue,
      }))) {
  ...
  const [specifyInstanceSchemaDialog, onSpecifyInstanceSchema] = useSpecifyInstanceSchemaDialog(false);
