...
export const initializationService: InitializationService = new InitializationService();
...
export function fromFieldToIFieldsGroup(field: Field): IAtlasmapGroup | null {
  const fields = field.children.map(fromFieldToIFields).filter((f) => f) as IField[];
...
export function fromFieldToIFieldsNode(field: Field): IField | null {
...
export function fromDocumentDefinitionToFieldGroup(def: DocumentDefinition): IDocument | null {
  ...
  const fields = def.fields.map(fromFieldToIFields).filter((f) => f) as IField[];
  const namespaces = def.namespaces.map(fromNamespaceModelToINamespace).filter((n) => n) as INamespace[];
...
export function fromMappedFieldToIMappingField(mappedField: MappedField): IMappedField | null {
  if (!mappedField.field || !mappedField.field.docDef) {
    return null;
  }
  return {
    id: `${
      mappedField.field!.docDef.uri || mappedField.field!.docDef.type + '-' + mappedField.field!.scope
    }:${mappedField.field!.docDef.isSource ? 'source' : 'target'}:${mappedField.field!.path}`,
    ...
    transformations: mappedField.actions.map((a) => ({
      ...
      arguments: a.definition!.arguments.map(
        (av, idx): ITransformationArgument => ({
          label: av.serviceObject?.title?.length > 0 ? av.serviceObject.title : CommonUtil.toDisplayable(av.name),
          type: av.type,
          name: av.name,
...
export function errorLevelToVariant(level: ErrorLevel): INotification['variant'] {
  switch (level) {
  ...
  return {
    ...
    title:
      e.mapping && e.type !== ErrorType.PREVIEW
        ? `${errorTypeToString(e.type)}: "${e.mapping.transition.getPrettyName()}"`
...
export function fromMappingModelToImapping(m: MappingModel | null | undefined): IMapping | null {
  return m
    ? {
        id: m.uuid,
...
export function fromMappingDefinitionToIMappings(def: MappingDefinition | null): IMapping[] {
  return def ? (def.mappings.map(fromMappingModelToImapping).filter((d) => d) as IMapping[]) : [];
}

export function executeFieldSearch(searchFilter: string, isSource: boolean): string[][] {
...
export function mappingExpressionClearText(nodeId?: string, startOffset?: number, endOffset?: number) {
...
export function mappingExpressionInsertText(str: string, nodeId?: string, offset?: number) {
...
export function mappingExpressionObservable(): Observable<any> | null {
  if (!initializationService.cfg.mappings?.activeMapping?.transition?.expression) {
    return null;
  }
  return initializationService.cfg.mappings.activeMapping.transition.expression.expressionUpdated$;
}
export function mappingExpressionRemoveField(tokenPosition?: string, offset?: number, removeNext?: boolean) {
...
export function onFieldPreviewChange(field: IField, value: string) {
...
export function handleActionChange(action: FieldAction, definition: FieldActionDefinition) {
 ...
 * @param target
 */
export function handleIndexChange(isSource: boolean, currentIndex: number, target: number | Field) {
  const cfg = ConfigModel.getConfig();
  const activeMapping = cfg.mappings?.activeMapping;
  if (!activeMapping) {
    return;
  }
  const sourceField = activeMapping.getMappedFieldForIndex('' + (currentIndex + 1), isSource);
  if (!sourceField) {
    return;
  }
...
export function handleNewTransformation(isSource: boolean, index: number) {
  ...
  const field = activeMapping.getMappedFieldForIndex('' + (index + 1), isSource);
  if (!field) {
    return;
  }
...
export function handleTransformationChange(
  ...
  const newAction = getMappingActions(isSource).find((a) => a.name === newTransformationName);
...
export function handleRemoveTransformation(isSource: boolean, index: number, transformationIndex: number) {
  const cfg = ConfigModel.getConfig();
  const activeMapping = cfg.mappings?.activeMapping;
  if (!activeMapping) {
    return;
...
export function handleMultiplicityChange(action: FieldAction, name: string) {
  ...
  const newAction = getMultiplicityActions(activeMapping).find((a) => a.name === name);
...
export function selectMapping(mapping: IMapping) {
