...
import { useNodeRect } from '../../../providers/NodeRefProvider';
...
export interface INodesArcProps extends Omit<Omit<Omit<IArcProps, 'start'>, 'end'>, 'type'> {
  start: string;
  end: string;
}

export const NodesArc: FunctionComponent<INodesArcProps> = ({ start: startId, end: endId, color, ...props }) => {
  ...
  const [linkType, setLinkType] = useState<'horizontal' | 'vertical'>('horizontal');
  ...
  const calculateCoords = useCallback(
    function calculateCoordsCb() {
      ...
      if (startRect && endRect) {
        const [smallRect, bigRect] = startRect.width < endRect.width ? [startRect, endRect] : [endRect, startRect];
        ...
        if (sameVerticalSpace) {
          const [aboveId, bottomId] = startRect.top > endRect.top ? [startId, endId] : [endId, startId];
          ...
          const [leftId, rightId] = startRect.left > endRect.left ? [startId, endId] : [endId, startId];
