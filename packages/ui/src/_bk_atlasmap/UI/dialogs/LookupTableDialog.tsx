...
*/
import { Form, FormGroup, FormSelect, FormSelectOption, Label } from '@patternfly/react-core';
...
export const LookupTableDialog: FunctionComponent<ILookupTableDialogProps> = ({
  ...
  const [targetEnum, setTargetEnum] = useState(enumerationValue.selectedTargetEnumValue);

  const onChangeTargetEnum = (enumValue: string, _event: React.FormEvent<HTMLSelectElement>) => {
  ...
  return (
    <Form>
      <FormGroup className={styles.iGroup} fieldId={'lookup-table-row'}>
        <Label className={styles.iSelectLabel}>{enumerationValue.sourceEnumValue}</Label>
        <FormSelect
        ...
        >
          {isOpen &&
            ...
            enumerationValue.targetEnumValues.map((value, idx) => (
              <FormSelectOption key={`tgtenum-${idx}`} value={value} label={value} />
            ))}
        </FormSelect>
      </FormGroup>
    </Form>
