...
export const CustomClassDialog: FunctionComponent<ICustomClassDialogProps> = ({
  ...
  const [customClassName, setCustomClassName] = useState(initialCustomClassName);
  ...
  return (
    ...
    >
      <Form>
        <FormGroup label={'Custom class name'} fieldId={'custom-class-name'} isRequired={true}>
          <FormSelect
          ...
          >
            {isOpen &&
              customClassNames &&
              customClassNames.map((value, idx) => <FormSelectOption key={idx} value={value} label={value} />)}
          </FormSelect>
        </FormGroup>
