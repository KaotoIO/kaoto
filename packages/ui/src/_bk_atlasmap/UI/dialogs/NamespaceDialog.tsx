...
export const NamespaceDialog: FunctionComponent<INamespaceDialogProps> = ({
  ...
  const [targetNamespace, setTargetNamespace] = useState(initialTargetNamespace);
    ...
    >
      <Form>
        <FormGroup label={'Alias'} fieldId={'alias'} isRequired={true}>
          <TextInput value={alias} onChange={setAlias} id={'alias'} autoFocus={true} isRequired={true} />
        </FormGroup>
        ...
        <FormGroup label={'Location URI'} fieldId={'locationUri'}>
          <TextInput value={locationUri} onChange={setLocationUri} id={'locationUri'} />
        </FormGroup>
