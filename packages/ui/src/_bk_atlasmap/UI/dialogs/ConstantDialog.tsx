...
export const ConstantDialog: FunctionComponent<IConstantDialogProps> = ({
  ...
  const [isConstantNameValid, setConstantNameValid] = useState(ValidatedOptions.default);
      ...
      <Form>
        <FormGroup label={'Name'} fieldId={'name'} isRequired={true}>
          {!isConstantNameUnique ? (
            <Tooltip content={<div>A constant with this name already exists</div>} entryDelay={750} exitDelay={100}>
              <TextInput
                value={name}
