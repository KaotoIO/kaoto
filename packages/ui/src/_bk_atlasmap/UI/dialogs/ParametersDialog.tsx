...
export const ParametersDialog: FunctionComponent<IParametersDialogProps> = ({
  ...
  const handleAddParameter = useCallback(() => {
    setDefinedParameters(definedParameters.concat({ ...availableParameters[0] }));
      ...
      <Form isHorizontal style={formLabelColumnWidth}>
            ...
            label={
              parameter.required ? (
                ...
                <FormSelect
                  ...
                  onChange={(availableParameterIndex) => handleChangeParameter(index, availableParameterIndex)}
                >
                  {[parameter].concat(availableParameters).map((parameter, index) => (
                    <FormSelectOption key={index} value={index} label={parameter.label} />
                  ))}
                </FormSelect>
              )
            }
        ...
        <span>
          <Button
            ...
            isDisabled={!availableParameters || availableParameters.length === 0}
