...
export interface IMappingTableProps {
  ...
  shouldShowMappingPreview: (field: IMapping) => boolean;
...
export const MappingTableView: FunctionComponent<IMappingTableProps> = ({
    ...
    mappings.length === 0
      ...
      : mappings.map((mapping) => {
          ...
          const targets = mapping.targetFields.map((target, index) => {
            const field: IField = target;
            const { name } = field;
            return (
              <div key={index}>
                {name}
                {shouldShowMappingPreview(mapping) && <DocumentFieldPreviewResults id={field.id} value={field.value} />}
              </div>
            );
          });
          return {
  ...
  const handleSelectMapping = (_event: any, row: IRow) => {
    const mapping: IMapping | undefined = mappings.find((mapping) => (row.cells?.[0] as ICell).data === mapping.id);
    if (mapping) {
  ...
  return (
    <MainContent>
      ...
      <Table aria-label="Mappings" cells={columns} rows={rows}>
        <TableHeader />
        <TableBody onRowClick={handleSelectMapping}></TableBody>
      </Table>
    </MainContent>
  );
};
