...
export interface IMappingDetailsViewProps {
  ...
  sources: Array<IMappedField | null>;
  targets: Array<IMappedField | null>;
  addableSources: IField[];
  addableTargets: IField[];
  ...
  onIndexChange: (isSource: boolean, currentIndex: number, newIndex: number | Field) => void;
  ...
  onRemoveTransformation: (isSource: boolean, index: number, currentTransformationIndex: number) => void;
  onAddFieldToMapping: (isSource: boolean, field: IField) => void;
  onNotificationRead: (id: string) => void;
  onEditEnum: (cb: any) => void;
  isEnumMapping: () => boolean;
  onCreateConstant: (constants: any | null, addToActiveMapping?: boolean) => void;
  onCreateProperty: (isSource: boolean, props: any | null, addToActiveMapping?: boolean) => void;
...
export const MappingDetailsView: FunctionComponent<IMappingDetailsViewProps> = ({
  ...
  const genericPaddingField: IField = {
    id: '',
    name: 'Padding field',
    value: '',
    type: '',
    ...
    isDisabled: false,
    amField: {} as IField['amField'],
    enumeration: false,
  };

  const renderMappingField = (isSource: boolean, canShowIndex: boolean, f: IMappedField | null, index: number) => {
    return f ? (
      <MappingField
        key={f.id}
        field={f}
        ...
        onIndexChange={(value: string | IField) =>
          typeof value === 'string'
        ...
        <div className={'pf-c-form'} style={formInputGroupStyle}>
          {f.transformations.map((t, transformationIndex) => (
            <MappingTransformation
              ...
              disableTransformation={mappingExpressionEnabled}
              onTransformationChange={(value) => onTransformationChange(isSource, index, transformationIndex, value)}
              onTransformationArgumentChange={(name, value) =>
                onTransformationArgumentChange(isSource, index, transformationIndex, name, value)
              }
              onRemoveTransformation={() => {
  ...
  const renderSourceMappingField = (f: IMappedField | null, index: number) =>
    renderMappingField(true, showSourcesIndex, f, index);
  const renderTargetMappingField = (f: IMappedField | null, index: number) =>
  ...
  const messages = notifications.filter((n) => n.variant !== 'warning' && n.variant !== 'danger');
  ...
  return (
    ...
    >
      {errors.length > 0 && (
        <NotificationsGroup notifications={errors} title={'Errors'} onNotificationRead={onNotificationRead} />
      )}
      {warnings.length > 0 && (
        <NotificationsGroup notifications={warnings} title={'Warnings'} onNotificationRead={onNotificationRead} />
      )}
      {messages.length > 0 && (
        <NotificationsGroup notifications={messages} title={'Messages'} onNotificationRead={onNotificationRead} />
      )}
      <div className={'pf-c-form'} style={formInputGroupStyle}>
        {mappingAction}
        <MappingFields
...
const NotificationsGroup: FunctionComponent<INotificationsGroupProps> = ({
  ...
  return (
    <Document title={title} noShadows={true} noPadding={true} actions={[<Badge key={1}>{notifications.length}</Badge>]}>
