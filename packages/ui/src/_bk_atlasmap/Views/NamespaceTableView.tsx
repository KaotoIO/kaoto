...
import { ICell, IRowData, textCenter } from '@patternfly/react-table';
...
export interface INamespaceTableProps {
  ...
  onEditNamespace: (docName: string, alias: string, uri: string, locationUri: string, isTarget: boolean) => void;
...
export const NamespaceTableView: FunctionComponent<INamespaceTableProps> = ({
  ...
  const namespaceTables: ReactElement[] = sources.reduce((xmlSources: ReactElement[], source: IDocument) => {
    if (source.type !== 'XML') {
      return xmlSources;
    }
    ...
    const actions = [
      {
        ...
        onClick: (_event: any, _rowId: any, row: IRowData, _extra: any) => {
          onEditNamespace(
            ...
            ((row.cells?.[3] as ICell).title as string).length > 0 ? true : false,
          );
        },
      },
      {
        title: 'Remove',
        onClick: (_event: any, _rowId: any, row: any, _extra: any) => onDeleteNamespace(source.name, row.alias.title),
      },
    ];
