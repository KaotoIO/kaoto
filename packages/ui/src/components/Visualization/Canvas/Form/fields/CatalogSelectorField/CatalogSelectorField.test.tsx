...
describe('CatalogSelectorField', () => {
  ...
  const renderField = (
    ...
    const propName = schema.title === 'Test Catalog' ? 'testingCatalog' : 'camelCatalog';
    const model = catalogValue ? { [propName]: catalogValue } : {};
      ...
      <RuntimeContext.Provider value={runtimeContextValue}>
        <SchemaProvider schema={schema}>
          <ModelContextProvider model={model} onPropertyChange={onPropertyChange}>
            <CatalogSelectorField propName={propName} />
  ...
  it.each([
    [
      'Camel',
      camelCatalogSchema,
      { version: '', runtime: 'Main' },
      'Camel Main 4.14.5',
      ['Main', 'Quarkus', 'Spring Boot'],
      ['Citrus'],
    ],
    ['Test', citrusCatalogSchema, { version: '', runtime: 'Citrus' }, 'Citrus 4.10.1', ['Citrus'], ['Main']],
  ])(
    'renders %s catalog selector with default catalog when version is empty',
    async (_type, schema, catalogValue, expectedDisplay, visibleRuntimes, hiddenRuntimes) => {
      const user = userEvent.setup();
      renderField(schema, catalogValue, mockCatalogLibrary);
      const toggle = screen.getByRole('button', { expanded: false });
      expect(toggle.textContent).toContain(expectedDisplay);

      await user.click(toggle);
      visibleRuntimes.forEach((runtime) => {
        expect(screen.getByText(runtime)).toBeInTheDocument();
      });
      hiddenRuntimes.forEach((runtime) => {
  ...
  it('handles catalog selection and saves only version and runtime', async () => {
    const user = userEvent.setup();
