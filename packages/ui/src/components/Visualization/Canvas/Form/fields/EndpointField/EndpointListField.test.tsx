...
describe('EndpointListField', () => {
  ...
  describe('deleting endpoints', () => {
    ...
    it('should not remove endpoint when deletion is canceled', async () => {
      ...
      // both httpClient should and jmsQueue should remain as deletion has been cancelled
      await waitFor(() => {
        expect(screen.queryByText('httpClient')).toBeInTheDocument();
        expect(screen.getByText('jmsQueue')).toBeInTheDocument();
      });

      // onPropertyChange should not be called as items did not change
