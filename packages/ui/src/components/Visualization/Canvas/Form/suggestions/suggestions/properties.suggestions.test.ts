import { propertiesSuggestionProvider } from './properties.suggestions';
...
describe('Properties Suggestions', () => {
  it.each([
    ...
    const applies = propertiesSuggestionProvider.appliesTo(propertyName, schema);
    expect(applies).toBe(expected);
  });

  it('should return suggestions', async () => {
    const word = 'test';
    const suggestions = await propertiesSuggestionProvider.getSuggestions(word, {
