...
export default function OpenApis() {
    ...
    const openApiDeleteToggle = useCallback((openApiId: string) => {
    ...
    const handleDeleteOpenApi = (openApiId: string) => {
        ...
        var newOpenApis = openApis.filter(openApi => openApi.id !== openApiId);
    ...
    useEffect(() => {
        var newOpenApis = new Array<OpenApi>();
    ...
    return(
        ...
        <Table borders={false} variant="compact">
            <Thead noWrap>
                ...
                <Tr>
                    <Th width={20}>ID</Th>
                    <Th width={60} colSpan={2}>Specification</Th>
            ...
            <Tbody>
                {filtered.map((openApi, index) => {
                    ...
                    return <Tr key={openApi.id}>
                                <Td>{openApi.id}</Td>
                                <Td colSpan={2}>{openApi.specification}</Td>
                                <Td>
                                    <ActionList>
                                        <ActionListItem key={openApi.id + '_action'}>
                                            <Icon onClick={() => handleDeleteOpenApi(openApi.id)}><DeleteIcon/></Icon>
                                        </ActionListItem>
                                    </ActionList>
                                </Td>
                            </Tr>;
                        ...
                        <Td colSpan={4}>
                            <Bullseye>
                                <EmptyState variant={EmptyStateVariant.sm}>
                                    ...
                                    <EmptyStateBody>No Open API matches your filter settings. Change your filter or add an Open API.</EmptyStateBody>
                                    <EmptyStateFooter>
                                        <EmptyStateActions>
