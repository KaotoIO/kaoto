...
import './ExpressionFieldSearch.css';
...
export const ExpressionFieldSearch: FunctionComponent<IExpressionFieldSearchProps> = ({
  ...
  function onKeyHandler(_index: number, _innerIndex: number, _position: string): void {
    // TODO
  }
  ...
  function selectionChanged(event: any, value: string | SelectOptionObject, isPlaceholder?: boolean | undefined): void {
    if (!isPlaceholder) {
      const docId = event.target.getAttribute('label');
  ...
  return (
    <div aria-label="Expression Field Search" className={styles.searchMenu} data-testid={'expression-field-search'}>
      <Select
        onToggle={toggle}
        isOpen={state}
        value={selectedField}
        ...
        data-testid={id}
      >
        {mappedFieldCandidates.map((s, idx: number) => createSelectOption(s, idx))}
      </Select>
    </div>
  );
};
