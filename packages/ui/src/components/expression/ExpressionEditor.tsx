...
*/
import { Button, Form, FormGroup, InputGroup, TextInput, Tooltip } from '@patternfly/react-core';
import { EnumValue, useToggle } from '../../_bk_atlasmap/impl/utils';
import { Observable, Subscription } from 'rxjs';
import React, { FunctionComponent, KeyboardEvent, MouseEvent, useCallback, useEffect } from 'react';

import { ExpressionEnumSelect } from './ExpressionEnumSelect';
import { ExpressionFieldSearch } from './ExpressionFieldSearch';
import { IExpressionNode } from '../../_bk_atlasmap/core';
...
export function initializeMappingExpression() {
  ...
  if (mappingExprObs) {
    expressionUpdatedSubscription = mappingExprObs.subscribe((updatedEvent: IExpressionUpdatedEvent) => {
...
export const ExpressionEditor: FunctionComponent<IExpressionContentProps> = ({
  executeFieldSearch,
  getFieldEnums,
  mappingExpressionAddField,
  ...
  let clearText: (nodeId?: string, startOffset?: number, endOffset?: number) => IExpressionNode | null;
  let fieldSearch: (searchFilter: string, isSource: boolean) => string[][];
  let getEnums: (enumFieldName: string) => EnumValue[];
  let setSelEnumValue: (selectedEnumNodeId: string, selectedEnumValueIndex: number) => void;

  const { state: showEnumSelect, toggleOn: toggleEnumSelOn, toggleOff: toggleEnumSelOff } = useToggle(false);
   ...
   */
  function onEnumSelect(selectedEnumNodeId: string, selectedEnumValueIndex: number) {
  ...
  function onChange(_event: React.FormEvent<HTMLDivElement>) {
    ...
    expressionUpdatedSubscription = mappingExprObservable()!.subscribe((updatedEvent: IExpressionUpdatedEvent) => {
      updateExpressionMarkup();
   ...
   * @returns
   */
  function insertSelectedField(selectedDocId: string, selectedField: string): void {
        ...
        <FormGroup fieldId="expressionContent">
          <InputGroup>
            ...
            >
              ...
              >
                <i>
                  f<small style={{ position: 'relative', bottom: -3 }}>(x)</small>
                </i>
              </Button>
            </Tooltip>
            {!disabled && mappingExpression !== undefined ? (
              ...
              <TextInput isDisabled={true} aria-label={'Expression content'} /> /* this to render a disabled field */
            )}
          </InputGroup>
        </FormGroup>
      </Form>
