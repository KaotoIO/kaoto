...
import './ExpressionEnumSelect.css';
...
export const ExpressionEnumSelect: FunctionComponent<IExpressionEnumSelectProps> = ({
  ...
  function createSelectOption(selectField: string, idx: number): any {
    if (selectField[1].length === 0) {
      return (
        <SelectOption isDisabled={true} label={selectField} value={selectField} key={idx} className={styles.document} />
      );
    } else {
      return <SelectOption label={selectField} value={selectField} key={idx} className={styles.field} />;
    }
  }

  return (
    <div aria-label="Expression Enumeration" className="enumSelectMenu" data-testid={'expression-enumeration-select'}>
      <Select onToggle={toggle} isOpen={state} value={selectValue} id={id} onSelect={selectionChanged} data-testid={id}>
        {enumCandidates.map((s, idx: number) => createSelectOption(s.name, idx))}
      </Select>
    </div>
  );
};
