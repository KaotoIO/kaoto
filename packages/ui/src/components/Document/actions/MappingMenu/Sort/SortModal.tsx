...
export const SortModal: FunctionComponent<SortModalProps> = ({ isOpen, onClose, mapping, onUpdate }) => {
  ...
  const initialEntries: SortKeyEntry[] =
    mapping.sortItems.length > 0
      ? mapping.sortItems.map((s) => {
          const cloned = new SortItem();
          cloned.expression = s.expression;
          cloned.order = s.order;
          return assignId(cloned);
        })
      : [assignId(new SortItem())];
  const [entries, setEntries] = useState<SortKeyEntry[]>(initialEntries);
  ...
  const handleExpressionChange = useCallback((index: number, value: string) => {
    setEntries((prev) => {
      ...
      item.order = updated[index].sortItem.order;
      updated[index] = { id: updated[index].id, sortItem: item };
      return updated;
    });
  }, []);

  const handleOrderToggle = useCallback((index: number) => {
    setEntries((prev) => {
      const updated = [...prev];
      const item = new SortItem();
      item.expression = updated[index].sortItem.expression;
      item.order = updated[index].sortItem.order === 'ascending' ? 'descending' : 'ascending';
      updated[index] = { id: updated[index].id, sortItem: item };
      return updated;
    });
  }, []);
  ...
  const handleDrop = useCallback(
    (_event: unknown, newItems: DraggableObject[]) => {
      isDraggingRef.current = false;
  ...
  return (
    ...
    >
      <ModalHeader title="Configure Sort" description={description} />
      <ModalBody>
        <Form>
