...
export function commonActions({
  ...
  return [
    <Tooltip key={'select-mapping'} position={'top'} enableFlip={true} content={<div>Show mapping details</div>}>
      {connectedMappings.length > 1 ? (
        <AutoDropdown
          toggle={(toggleRef) => (
            <MenuToggle ref={toggleRef} aria-label="Show mapping details">
              <ExchangeAltIcon />
            </MenuToggle>
          )}
          isPlain={true}
    ...
    <Tooltip key={'add'} position={'top'} enableFlip={true} content={<div>Create new mapping</div>}>
      <Button
        variant="plain"
