...
> = ({ getResourcesContentByType, children }) => {
  ...
  function fetchCitrusCatalog(basePath: string) {
    // Deduce Citrus catalog path from Camel catalog path
    const citrusCatalogPath = basePath.replace('camel-catalog', 'citrus-catalog');

    // ToDo: make Citrus catalog selectable
    const indexFile = `${citrusCatalogPath}/citrus/index.json`; // latest catalog version
    const relativeBasePath = CatalogSchemaLoader.getRelativeBasePath(indexFile);
    fetch(indexFile)
      .then((response) => {
        setLoadingStatus(LoadingStatus.Loading);
      ...
      .then(async (catalogIndex: CitrusCatalogIndex) => {
        ...
        const endpointFiles = CatalogSchemaLoader.fetchFile<ComponentsCatalog[CatalogKind.TestEndpoint]>(
          `${relativeBasePath}/citrus-catalog-aggregate-endpoints.json`, // ToDo: use catalog index file when Citrus 4.9.2 is available
        );
        /** Citrus test functions */
        const functionFiles = CatalogSchemaLoader.fetchFile<ComponentsCatalog[CatalogKind.TestFunction]>(
          `${relativeBasePath}/citrus-catalog-aggregate-functions.json`, // ToDo: use catalog index file when Citrus 4.9.2 is available
        );
        ...
        >(`${relativeBasePath}/citrus-catalog-aggregate-validation-matcher.json`); // ToDo: use catalog index file when Citrus 4.9.2 is available
