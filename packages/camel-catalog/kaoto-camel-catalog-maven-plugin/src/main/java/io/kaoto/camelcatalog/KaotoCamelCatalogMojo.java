...
public class KaotoCamelCatalogMojo extends AbstractMojo {
    ...
    private ObjectNode yamlDslSchema;
    ...
    public void execute() {
        ...
        var index = new Index();
        var yamlDslSchema = processCamelSchema(path, index);
        processK8sSchema(path, index);
        processCatalog(path, index, yamlDslSchema);
    ...
    private ObjectNode processCamelSchema(Path inputDir, Index index) {
    ...
    private void processCatalog(Path inputDir, Index index, ObjectNode yamlDslSchema) {
        var catalogProcessor = new CamelCatalogProcessor();
        try {
            var catalogMap = catalogProcessor.processCatalog(yamlDslSchema);
