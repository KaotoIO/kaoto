...
import catalogLibraryIndex from '../dist/index.json' assert { type: 'json' };
...
async function main() {
  ...
  const indexFileUri = pathToFileURL(`./dist/${indexDefinitionFileName}`).toString();
  ...
  const schemaPromises = Object.entries(indexDefinitionContent.schemas).map(async ([name, schema]) => {
    ...
    const schemaFile = resolve(`./dist/${baseFolder}/${schema.file}`);
